...
inline void scoring_gk_curr(int &k,size_t &kmerPerLig,unsigned &currLigLen,const vector<unsigned long> &k_code_currLig,vector<IntegerVector> &allkShiftCombis,IntegerVector allGapCombiLens, vector<double> &scores, bool posInfo=true)
{
  for (size_t currPos=0; currPos< kmerPerLig;currPos++) //cnt gk for each pos
  {
    size_t combiNo= allkShiftCombis.size(); //gap combi num
...
)
{
  ...
  for (unsigned i = 0; i < ligNo; i++)
  {
    // if (diffLen) {ligLens= strings[i].length();  kmerPerLig= ligLens -km1;}
    const vector<unsigned long> k_code_currLig= count_k_curr(strings[i],kmerPerLig,k,km1,currN_val,kmerMask);
    scoring_gk_curr(k,kmerPerLig,ligLens,k_code_currLig,allkShiftCombis,allGapCombiLens,scores,posInfo);
  }
